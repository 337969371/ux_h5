...
export default class Article extends React.Component {
    ...
    componentDidMount() {
        this.props.getDetail({articleId:this.props.location.query.id}).then(res=>{
            this.setState({
                loading:false,
            })
        });
    ...
    isLike(){
        ...
        this.props.isLike({
            ...
            this.props.getDetail({articleId:id});
        })
    ...
    comment(){
        ...
        this.props.comment(param).then((res)=>{
    ...
    share(bool){
        const { article,forward } =this.props;
        let detail = article.detail;
        this.setState({
            loading:true,
        })
        forward({
            imageUrl: detail.imageUrl,  //帖子图片链接
    ...
    render() {
        ...
        const { article,globalData } =this.props;
        let commentList = [];
        if(article.comment && article.comment.records){
            commentList = article.comment.records;
        }
        let detail = article.detail;
        if(!detail){
            return null;
        ...
        return (
            <div className="article-detail">
                <HeaderNav transparent={true} {...this.props}  isLight={true}/>
                <Login {...this.props}/>
                {loading &&
                <ActivityIndicator
                    toast
                ...
                <div className="article-detail-slider-img-box">
                    {detail.videoUrl?<div className="article-detail-slider-img">
                        <video  controls poster={detail.forwardImageUrl} >
                            <source src={detail.videoUrl} type="video/mp4" />
                        </video>
                    </div>:<div className="article-detail-slider-img">
                ...
                <div className="post-content">
                    ...
                    <div className="article-detail-content-text" dangerouslySetInnerHTML={{__html:detail.content}}>
                    </div>
                    <div>
                ...
                {showEdit && <div className="mask">
                    ...
                    <div className="post-comment-push">
                        ...
                        <div className="post-comment-push-btn">
                            ...
                            <button onClick={()=>this.comment()}>发表</button>
                        </div>
                    </div>
                </div>}
            </div>
