...
* Cleaned up `connect` decorator in `PostPush`
* Cleaned up flow types in `PostPush`
...
* Added example `composes` style to demo CSS modules in `PostPush`
...
#### Fixes
* Fixes naming in `PostPush` where `mapStateToProps` was incorrectly written as `mapDispatchToProps`.
